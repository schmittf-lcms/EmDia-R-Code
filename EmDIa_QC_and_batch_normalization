...
#data import - MAC
emdia <- "/Users/filepath_to_csv/Emdia MS1 RAW data.csv" 
emdia <- read.csv2(emdia, header = TRUE, dec = ".", sep = ";")
head(data_raw)

#data import - Windows
emdia <- "C:/Users/filepath_to_csv/Emdia MS1 RAW data.csv" 
emdia <- read.csv2(emdia, header = TRUE, dec = ".", sep = ",")
head(data_GHS)

###
###
###PCA of non-corrected EmDia data
###
###

emdia_transposed <- as.data.frame(t(emdia))
#set name of column
emdia_transposed <- setNames(data.frame(t(emdia[,-1])), emdia[,1])
...
autoplot(emdia_transposed_values_pca, data = emdia_transposed, colour = "batch", shape = "class", size = 4)+
  ggtitle("PCA of non-corrected EmDia data")+
...
###
###
###CVs and Violin Plots of non-corrected EmDia data
###
###
...
ggplot(cv_data, aes(x = class, y = CV, fill = batch)) +
  ...
  labs(
    title = "Coefficient of Variation of Metabolites by Batch and Class non-corrected EmDia data",
    x = "Class",
...
emdia_t= setNames(data.frame(t(emdia[,-1])), emdia[,1])
...
write.table(emdia_corrected_data, 
            file = "C:/Users/filepath_to_csv/emdia_corrected.csv", 
            sep = ",", 
            dec = ".", 
            row.names = TRUE, 
            col.names = TRUE)
###
###
###PCA of corrected EmDia data
###
###

emdia_corrected_data <- "C:/Users/filepath_to_csv/emdia_corrected.csv" 
...
autoplot(pca_res, data = emdia_corrected_data_transposed, colour = "batch", shape = "class", size = 4)+
  ggtitle("PCA of batch-corrected EmDia data")+
  theme(axis.text.x = element_text(size=12, angle = 45, vjust = 1, hjust=1), legend.position = "right") +
  theme(legend.text=element_text(size=20))+
  theme(panel.background = element_rect(fill = "white"),
        panel.grid.major = element_line(color = "grey", linetype = "dotted", linewidth = 0.75))+
...
ggplot(cv_data, aes(x = class, y = CV, fill = batch)) +
  ...
  scale_color_manual(values = c("Treatment" = "red", "Placebo" = "blue")) +
  labs(
    title = "Coefficient of Variation of Metabolites by Batch and Class batch-corrected EmDia data",
    x = "Class",
    y = "Coefficient of Variation (%)"
  ) +
  theme_minimal() +
